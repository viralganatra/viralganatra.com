...
_If you're using Yarn rather than NPM you can just continue with `yarn install` as normal._
...
_For those of you eagle-eyed readers we no longer need to specify a Docker Compose version since both v2 and v3 properties are now supported and is the recommended way to go moving forward._
...
*One thing to note is EXPOSE doesn't publish any ports; it only exists as a type of documentation for anyone reading the Dockerfile. The ports section in the Docker Compose file does the actual publishing.*
